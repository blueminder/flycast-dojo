		...
		if (bytes_read)
		{
			...
			if (dojo.host_status == 4 && memcmp("JOINED", buffer, 6) == 0)
			{
				...
				auto it = std::find_if(dojo.presence.players.begin(), dojo.presence.players.end(),
					[player_ip, player_ggpo_port](Player& p){ return p.ip == player_ip && p.ggpo_port == player_ggpo_port; } );
			...
			if (dojo.host_status == 1 && memcmp("JOIN", buffer, 4) == 0)
			{
				...
				if (dojo.presence.targets.count(current_target) == 1)
					continue;
				...
				std::string response = std::string("JOINED " + std::to_string(player.port_num) + ":" + player.name + ":" + player.ip + ":" + std::to_string(player.ggpo_port));
...
{
	if(dojo.presence.player_count == config::NumPlayers.get())
	{
		if (config::NumPlayers == 2)
			config::OpponentName = dojo.presence.player_names[1];
		else
			config::OpponentName = std::accumulate(dojo.presence.player_names.begin(), dojo.presence.player_names.end(), std::string("_"));
		...
		try
		{
			...
			else if (config::NumPlayers > 2)
			{
				...
				config::NetworkP0Server = dojo.presence.player_ips[0];
				config::GGPOP0Port = dojo.presence.player_ggpo_ports[0];
				config::NetworkP1Server = dojo.presence.player_ips[1];
				config::GGPOP1Port = dojo.presence.player_ggpo_ports[1];
			}

			if (config::NumPlayers == 3)
			{
				config::NetworkP2Server = dojo.presence.player_ips[2];
				config::GGPOP2Port = dojo.presence.player_ggpo_ports[2];
			}
			else if (config::NumPlayers == 4)
			{
				config::NetworkP2Server = dojo.presence.player_ips[2];
				config::GGPOP2Port = dojo.presence.player_ggpo_ports[2];
				config::NetworkP3Server = dojo.presence.player_ips[3];
				config::GGPOP3Port = dojo.presence.player_ggpo_ports[3];
			}
			...
			std::cout << config::NetworkP0Server.get() << ":" << std::to_string(config::GGPOP0Port) << std::endl;
			std::cout << config::NetworkP1Server.get() << ":" << std::to_string(config::GGPOP1Port) << std::endl;
			std::cout << config::NetworkP2Server.get() << ":" << std::to_string(config::GGPOP2Port) << std::endl;
