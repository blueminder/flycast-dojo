...
void gdb_server_interrupt(gdb_server_t *sv, int signal, const char *info);
void gdb_server_process_ended(gdb_server_t *sv, int rc);
...
//
void gdb_server_interrupt(gdb_server_t *sv, int signal, const char *info = NULL)
{
	if (info != NULL)
	{
		char *scratch = scratch_buffer;
		*scratch++ = 'O';
		for (; *info; scratch += 2)
			sprintf(scratch, "%02x", *info++);
		gdb_server_send_packet(sv, scratch_buffer);
	}
	snprintf(scratch_buffer, sizeof(scratch_buffer), "S%02x", signal);
	gdb_server_send_packet(sv, scratch_buffer);
}

//
// tell the client that the process has ended
//
void gdb_server_process_ended(gdb_server_t *sv, int rc) {
  snprintf(scratch_buffer, sizeof(scratch_buffer), "W%02x", rc);
  gdb_server_send_packet(sv, scratch_buffer);
}

//
// tell the client that we've halted due to a watchpoint
//
void gdb_server_watchpoint_hit(gdb_server_t *sv, int signal, const char *watchpoint_type, int addr) {
  snprintf(scratch_buffer, sizeof(scratch_buffer), "T%02x%swatch:%x;", signal, watchpoint_type, addr);
  gdb_server_send_packet(sv, scratch_buffer);
}
...
static int gdb_server_handle_c(gdb_server_t *sv, const char *data) {
	if (data[0] == 'C')
	{
		// parse signal number (ignored)
		int signum;
		data = parse_hex(&data[1], &signum);
	}
	int addr = 0;
	if (*data)
...
// 'z type,addr,kind'
// Remove a type breakpoint or watchpoint starting at address address of given length
// kind.
...
//	gdb_server_send_packet(sv, "O486f6c64206f6e");	// Hold on
//	gdb_server_send_packet(sv, "O");
